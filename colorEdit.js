...
function Color()
{
   ...
   {
      return "#"+this.color[idx][0].toString(16)
                +this.color[idx][1].toString(16)
                +this.color[idx][2].toString(16)
   }

   this.updateResult=function()
   {
      var r="Editing: "+aliases[this.amino].amino+" (<span style='color: "
            +this.colorCode(0)+"; background: "
            +this.colorCode(1)+";'>"+aliases[this.amino].abbr+"</span>) "
            +"<a style='font-size: smaller;' "
            +"href='#' onclick='color.apply()'>apply changes</a>"
   ...
   this.interpret=function()
   {
      ...
      try {
         ...
         alert("Couldn't interpret the color table code because\n"
